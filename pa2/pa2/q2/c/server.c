...
/*
NOTE: run server using "sudo perf stat ./server <port_no>"
change the client port number to the port number used to setup server in above line.
compile and run client. pass the number of clients as a argument. command: "./client <num_clients"
*/
...
int main(int argc, char* argv[]) {
    ...
    printf("Server setup successful, listening on port %d\n", port);
        ...
        if (FD_ISSET(server_socket, &readfds)) {
            ...
            for (int i = 0; i < MAX_CLIENTS; i++) {
                if (client_sockets[i] == 0) {
                    ...
                    printf("Adding new client socket to list as %d\n", i);
                    break;
                }
            }
        }
